...
<?php
            ...
            $nomeDivida = ['nome_entrada_dependent'];
            $valor = ['valor_entrada_dependent'];
            ...
            $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_entradas_dependente WHERE id_usuario = $id_usuario ORDER BY valor_entrada_dependente DESC LIMIT 5");
			$sql->execute();
...
<main>
    <form method="post">
        ...
        if(isset($_POST['confirmar_dado'])){
            ...
            $data2 = $_POST['data_2'];
            if($data1 > 0){
                $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_entradas_dependente WHERE id_usuario = $id_usuario AND data_entrada_dependente BETWEEN CONCAT('".$data1."') AND CONCAT('".$data2."')");
                $sql->execute();
                $info1 = $sql->fetchAll();
            }else{
                $data1 = '2022-01-01';
    ...
    <section class="grafcbar_content grid">
                ...
                function drawColColors() {
                    var data = new google.visualization.arrayToDataTable([
                        ...
                        <?php foreach ($info1 as $key => $value){?>                       
                            ['<?php echo $value['nome_entrada_dependente'];?>', <?php echo $value['valor_entrada_dependente']?>, ''],
                                   <?php } ?>
                    ]);
        ...
        <?php 
            if(isset($_POST['confirmar_dado'])){
                ...
                $data4 = $_POST['data_4'];                
                if($data3 > 0){
                    $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_entradas_dependente WHERE id_usuario = $id_usuario AND data_entrada_dependente BETWEEN CONCAT('".$data3."') AND CONCAT('".$data4."')");
                    $sql->execute();
                    $info2 = $sql->fetchAll();
                }else{
                    $data3 = '2022-01-01';
                    ...
                    <div id="chart_wrap3">
                        ...
                        <div class="Relatorio">
                            ...
                            <?php foreach ($topentradas as $key => $value){?>                                   
                                <h2><?php echo $value['nome_entrada_dependente'];?>: R$ <?php echo $value['valor_entrada_dependente']?></h2>
                                   <?php } ?>
                        </div><!--Relatorio-->
                </div>
