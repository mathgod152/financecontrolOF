...
<head>
    ...
    <link href="<?php echo INCLUDE_PATH_STATIC ?>estilos/44.css" rel="stylesheet">
...
<body>
    ...
    <?php
            ...
            $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_dividas_fixas WHERE id_usuario = $id_usuario ORDER BY valor_dividasF DESC LIMIT 5");
			$sql->execute();
...
<form method="post">
        ...
        if(isset($_POST['confirmar_dado'])){
            ...
            $data2 = $_POST['data_2'];
            if($data1 > 0){
                $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_dividas_fixas WHERE id_usuario = $id_usuario AND data_incremento_dividasF BETWEEN CONCAT('".$data1."') AND CONCAT('".$data2."')");
                $sql->execute();
                $info1 = $sql->fetchAll();
            }else{
                $data1 = '2022-01-01';
    ...
    <section class="grafcbar_content grid">
                ...
                function drawColColors() {
                    var data = new google.visualization.arrayToDataTable([
                        ...
                        <?php foreach ($info1 as $key => $value){?>                       
                                     ['<?php echo $value['nome_dividaF'];?>', <?php echo $value['valor_dividasF']?>, ''],
                                   <?php } ?>
                    ]);
        ...
        if(isset($_POST['confirmar_dado'])){
            ...
            $data4 = $_POST['data_4'];
            if($data3 > 0){
                $sql = \FinanceControl\MySql::connect()->prepare("SELECT * FROM tb_dividas_fixas WHERE id_usuario = $id_usuario AND data_incremento_dividasF BETWEEN CONCAT('".$data3."') AND CONCAT('".$data4."')");
                $sql->execute();
                $info2 = $sql->fetchAll();
            }else{
                $data3 = '2022-01-01';
    ...
    <div class="linegrafc_items" style="margin-top:-20px;">
                 <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
                     <script type="text/javascript">
                        ...
                        ['Divida', 'Valor', { role: 'style' }  ],
                        <?php foreach ($info as $key => $value){?>                       
                                     ['<?php echo $value['nome_dividaF'];?>', <?php echo $value['valor_dividasF']?>, 'ff9966'],
                                   <?php } ?>
                    ]);
                ...
                <div id="chart_wrap1">
                    ...
                    <div id="chart_wrap3">
                        ...
                        <div class="Relatorio">
                            <h1>Relatorio Maiores Dividas</h1>
                            <?php foreach ($topentradas as $key => $value){?>                                   
                                <h2><?php echo $value['nome_dividaF'];?>: R$ <?php echo $value['valor_dividasF']?></h2>
                                   <?php } ?>
                        </div><!--Relatorio-->
                </div>
                    </div>
